...
class UserImages extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('user_id, hash_file', 'required'),
			array('user_id', 'numerical', 'integerOnly'=>true),
			array('name, hash_file', 'length', 'max'=>255),
			array('title, description', 'length', 'max' => 255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, name, hash_file, title, description', 'safe', 'on'=>'search'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'user_id' => 'link table users',
			'name' => 'correct name with extension',
			'hash_file' => 'Hash File',
            'title' => 'Title',
